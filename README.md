...
This one is inspired by burrowx, but simplified with the more simple and robust logic I could think of.
...
| Metric | Description |
| --- | --- |
| `yakle_group_topic_partition_offset_°la{group, topic, partition}` | Number of messages the consumer group is behind for a given partition. |
| `yakle_group_topic_partition_time_°lga{group, topic, partition}` |Time in millisecond the consumer group is behind for a given partition. |
